...
describe("Protocol Test", () => {
  ...
  function startPrimaryStub(done) {
  ...
  before((done) => {
    startPrimaryStub(() => {
      rosnodejs.initNode(nodeName, initArgs).then(() => {
        done();
      });
    });
  ...
  describe("Pub-Sub", () => {
    ...
    it("Invalid Without Resolve Causes Error", (done) => {
      ...
      const logCapture = {
        write(rec) {
          if (
            rec.level === rosnodejs.log.levelFromName["error"] &&
    ...
    it("Shutdown Subscriber Connecting to Publisher", function (done) {
      ...
      sub.on("registered", () => {
        ...
        subImpl._handleTopicRequestResponse(
          [1, "ok", ["TCPROS", "junk_address", 1234]],
          "http://junk_address:1234"
        );
        sub.shutdown();
      });
    });
  ...
  describe("Service", () => {
    ...
    it("Service Shutdown During Call", function (done) {
      ...
      const client = nh.serviceClient(service, srvType);
      nh.waitForService(service).then(() => {
        client.call({});
      });
    ...
    it("Service Unregistered During Call", (done) => {
      // simulate a service disconnecting between the lookupService call to ROS Primary
          ...
          net.connect = (info) => {
            ...
            process.nextTick(() => {
              const error = new Error(
                `connect ECONNREFUSED ${info.host}:${info.port}`
              );
  ...
  describe("Shutdown", () => {
    ...
    before(() => {
      stopPrimaryStub(() => {
        primaryStub = new PrimaryStub("localhost", PRIMARY_PORT);
        primaryStub.provideAll();
      });

      return rosnodejs.shutdown().then(() => {
        rosnodejs.reset();
      });
    });
    ...
    it("Shutdown after successful start with primary running", function (done) {
      rosnodejs
        .initNode(nodeName, initArgs)
        .then(() => {
    ...
    it("Shutdown after successful start with primary down", function (done) {
      rosnodejs
        .initNode(nodeName, initArgs)
        .then(() => {
    ...
    it("Shutdown when unable to connect to primary", function (done) {
      rosnodejs.initNode(nodeName, initArgs).then(() => {
    ...
    it("Spinner is cleared out when shutdown", function (done) {
      ...
      rosnodejs.initNode(nodeName, initArgs).then((nh) => {
  ...
  describe("Parameters", function () {
    ...
    it("Set", function (done) {
      ...
      nh.setParam("/key", 2).then(() => {
        expect(primaryStub._params["/key"]).to.equal(2);
        done();
      });
    });
    ...
    it("Delete", function (done) {
      ...
      nh.deleteParam("/key").then(() => {
        expect(primaryStub._params["/key"]).to.be.undefined;
        done();
      });
    });
    ...
    it("Full", function (done) {
      ...
      nh.getParam("/missing")
        ...
        .then(() => {
          expect(primaryStub._params["/exists"]).to.equal(1);
  ...
  describe("initialization", () => {
    ...
    it("wait forever", function (done) {
      ...
      rosnodejs.initNode("test_node", { rosPrimaryUri }).then(() => {
        process.nextTick(() => {
    ...
    it("wait a little", function (done) {
      ...
      let timeout;
      rosnodejs
        .initNode("test_node", { rosPrimaryUri, timeout: 1000 })
        .then(() => {
          process.nextTick(() => {
            throw new Error("Node shouldnt initialize");
          });
    ...
    it("wait once", function (done) {
      ...
      let timeout;
      rosnodejs
        .initNode("test_node", { rosPrimaryUri, timeout: 0 })
        .then(() => {
          process.nextTick(() => {
            throw new Error("Node shouldnt initialize");
          });
