...
const Primary = require("./utils/PrimaryStub.js");
...
describe("OnTheFly", function () {
  ...
  before(function () {
    ...
    primary = new Primary("localhost", PRIMARY_PORT);
    primary.provideAll();
  ...
  it("serialize/deserialize PoseWithCovariance", (done) => {
    ...
    expect(
      read.covariance.length == msg.covariance.length &&
        read.covariance.every((v, i) => v === msg.covariance[i])
    ).to.be.true;
