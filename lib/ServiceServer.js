...
class ServiceServer extends EventEmitter {
  ...
  _handleMessage(client, data) {
    ...
    //console.log('SERVICE CALLBACK SUCCESS?? %s', success);
    //console.log('%j', resp);
    ...
    if (success) {
      ...
      this._messageHandler.Response.serialize(resp, serializedResponse, 5);
    }
    else {
    ...
    // console.log('SERVICE RESPONSE %s', serializedResponse.toString());
