...
class ServiceClient extends EventEmitter {
  ...
  call(request) {
    ...
    .then((msg) => {
      //console.log('call complete %j', msg);
      this._calling = false;
      return msg;
    })
    .catch((err) => {
    ...
    return new Promise((resolve, reject) => {
      deserializer.on('message', (msg, success) => {
        if (!client.$initialized) {
          ...
          //console.log('service client got header: %j', header);

          // serialize request
          const msgSize = this._messageHandler.Request.getMessageSize(request);
          //console.log('req size: ' + msgSize);

          // extra 4 bytes is for message size
          const serializedRequest = new Buffer(4 + msgSize);
          base_serializers.uint32(msgSize, serializedRequest, 0);
          this._messageHandler.Request.serialize(request, serializedRequest, 4);
          //console.log('SERIALIZED REQ %s', serializedRequest.toString('hex'));
          ...
          //console.log('GOT SERVICE %s MSG %s', success, msg.toString());
          // this is the actual response
          if (success) {
            resolve(this._messageHandler.Response.deserialize(msg, [0]));
          }
          else {
