...
class ServiceClient extends EventEmitter {
  ...
  call(request) {
    ...
    .catch((err) => {
      this._log.warn('Error during service call ' + err.stack);
      this._calling = false;
      throw err;
    });
  }
  ...
  _sendRequest(request) {
    // serialize request
    const serializedRequest = TcprosUtils.serializeMessage(this._messageHandler.Request, request);

    this._serviceClient.write(serializedRequest);
