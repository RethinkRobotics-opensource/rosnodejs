...
class ServiceClient extends EventEmitter {
  constructor(options, nodeHandle) {
    ...
    if (!options.typeClass) {
      throw new Error(
        `Unable to load service for service client ${this.getService()} with type ${this.getType()}`
      );
    }
  ...
  call(request) {
    return new Promise((resolve, reject) => {
      ...
      if (
        ...
        this._callQueue.length > this._maxQueueLength
      ) {
      ...
      .catch((err) => {
        if (!this.isShutdown()) {
          ...
          if (err.code !== "EROSAPIERROR") {
            this._log.error(
              `Error during service ${this.getService()} call ${err}`
            );
          }
  ...
  _sendRequest(call) {
    ...
    // serialize request
    const serializedRequest = TcprosUtils.serializeMessage(
    ...
    return new Promise((resolve, reject) => {
      const closeHandler = () => {
        this._log.debug(
          "Service %s client disconnected during call!",
          this.getService()
        );
  ...
  _connectToService(serviceHost, call) {
    return new Promise((resolve, reject) => {
      this._log.debug(
        "Service client %s connecting to %j",
        this.getService(),
        serviceHost
      );
  ...
  _createCallSocketAndHandlers(serviceHost, call, reject) {
    ...
    call.serviceClient = net.connect(serviceHost, () => {
      ...
      this._log.debug(
        "Sending service client %s connection header",
        this.getService()
      );

      let serviceClientHeader = TcprosUtils.createServiceClientHeader(
  ...
  _cacheSocketIfPersistent(call) {
    ...
    // Cache the service client for later use. Future calls won't need to lookup the service with the ROS primary
