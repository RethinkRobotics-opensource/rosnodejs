...
var CMAKE_PREFIX_PATH = process.env.CMAKE_PREFIX_PATH;
var cmakePaths = CMAKE_PREFIX_PATH.split(path.delimiter);
var jsMsgPath = path.join('share', 'gennodejs', 'ros');
...
//-----------------------------------------------------------------------
var packagePaths = {};
cmakePaths.forEach(function (cmakePath) {
  var dirPath = path.join(cmakePath, jsMsgPath);
  try {
    var msgPackages = fs.readdirSync(dirPath);
    msgPackages.forEach(function (msgPackage) {
...
module.exports = {
  Find: function Find(messagePackage) {
